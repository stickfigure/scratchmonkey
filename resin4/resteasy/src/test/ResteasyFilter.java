 ...
 * container can serve /my.jpg. The way this works:
 * </p>
 * <ol>
 ...
 * <li>When a request comes in, this filter first passes it through to the
 * container. If the container can service the request (not 404), great.</li>
 * <li>If the container returns 404, this filter dispatches the request to the
 * Resteasy servlet (which might subsequently return 404).</li>
 * </ol>
 * <p>
...
public class ResteasyFilter implements Filter
{
			...
			throws IOException, ServletException
	{
		...
		// If a SC_NOT_FOUND error occurred, then run the request through resteasy
		Integer errorCode = wrapper.getErrorCode();
		if (errorCode != null && errorCode == HttpServletResponse.SC_NOT_FOUND)
		{
			this.resteasyServlet.service(request, response);
		}
		else
		{
			wrapper.proceed();
